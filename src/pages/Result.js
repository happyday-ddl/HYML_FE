...
const ANIMAL_DATA = {
  ...
  "Jellyfish": {
    ...
    parallels: "Jellyfish are among the oldest animals on Earth — more than 500 million years of survival. Their simplicity is not a flaw; it is precisely why they outlast almost everything else. You endure by refusing to be brittle, by bending where others break.",
    interactions: {
...
// ─── Helpers ─────────────────────────────────────────────────────────────────
function toX(lon) { return ((lon - MAP.lonMin) / (MAP.lonMax - MAP.lonMin)) * MAP.W; }
function toY(lat) { return (1 - (lat - MAP.latMin) / (MAP.latMax - MAP.latMin)) * MAP.H; }
...
export default function Result() {
  ...
  async function handleSignupSubmit(e) {
    ...
    try {
      // 1. Try to sign up or login the user
      ...
      // Depending on your saveResult signature it might need userName or not.
      try {
      ...
      // 3. Cache locally so animalEmoji survives page navigation
      ...
      // 4. Stay on result page, show success state, update nav buttons
      setShowSignup(false);
      setJoined(true);
